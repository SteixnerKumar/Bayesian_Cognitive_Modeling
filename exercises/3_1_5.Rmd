...
# Question
Exercise 3.1.5 Alter the data to k = 99 and n = 100, and comment on the shape
of the posterior for the rate $\theta$.

# Comments/Solution
Comparing at the posterior distributions (from the plots section below) of k=99, n=100 to the previous distribution of k=50, n=100, one can observe that the $\theta$ estimates have shifted towards the extreme right of the graph to a value very close to 1 with slight uncertanity. Since the data shows that 99 times out of 100 we get a positive, the posterior has very little doubt as to where the real value lies.
...
mod_fit_1 <- stan(file="3_1_5.stan",   
                data=stan_data_1, 
                init=myinits,  # If not specified, gives random inits
                pars=parameters,
                iter=2000, 
...
```
For dataset k=99 and n=100. (stan_data_1)
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE}
# The commands below are useful for a quick overview:
